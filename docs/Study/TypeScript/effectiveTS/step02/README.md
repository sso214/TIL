...
* **객체 리터럴을 변수에 할당하거나 매개변수로 전달 시 잉여 속성 체크가 수행됨**
  * 타입이 명시된 변수에 객체 리터럴 할당 시 TS는 해당 타입의 속성이 있는지,   
    그리고 그 외의 속성은 없는지 확인함  
  * 하지만 구조적 타이핑 관점으로 생각해보면 그 외의 속성이 있어도 오류가 발생하지 않아야 함.  
    객체 리터럴은 해당 타입의 부분 집합을 포함하므로 해당 변수에 할당 가능하며 타입체커도 통과함  
  * 위의 두 차이는 객체 리터럴을 변수에 할당 시에는 구조적 타입 시스템에서 발생할 수 있는   
    중요한 종류의 오류를 잡을 수 있도록 '잉여 속성 체크'라는 과정이 수행되기 때문  
    =>   
    하지만 잉여 속성 체크 역시 조건에 따라 동작하지 않는다는 한계 존재.  
    통상적인 할당 가능 검사와 함께 쓰이면 구조적 타이핑이 무엇인지 혼란해질 수 있음  
    **잉여 속성 체크는 할당 가능 검사와는 별도의 과정임**  
  * 아래 Options 타입은 범위가 매우 넓기 때문에 순수한 구조적 타입 체커는   
    매개변수로 전달 시의 오타라던가 이런 종류의 오류를 찾아내지 못함      
    `inteface Options { title: string; darkMode?: boolean }`  
    darkMode 속성에 boolean 타입이 아닌 값이 지정된 경우를 제외하면   
    string 타입을 가진 title 속성과 또 다른 어떤 속성을 가지는 모든 객체는 Options 타입의 범위에 속함  
    (ex, document, new HTMLAnchorElement 인스턴스는 모두   
    string 타입인 title 속성을 가지고 있기 때문에 Options 타입 변수에 할당 가능함)  
    =>      
    따라 TS 타입은 범위가 아주 넓어질 수 있음  
  * 잉여 속성 체크를 이용하면 기본적으로 타입 시스템의 구조적 본질을 해치지 않으면서도  
    객체 리터럴에 알 수 없는 속성을 허용치 않음으로써, 앞의 문제점을 방지할 수 있음  
    (그래서 '엄격한 객체 리터럴 체크'라고도 불림)    
    객체 리터럴이 아닌 경우, 또는 타입 단언문을 사용한 경우 잉여 속성 체크가 적용되지 않고 오류 사라짐  
  * 잉여 속성 체크를 원치 않는다면, 인덱스 시그니처를 사용해 TS가 추가적인 속성을 예상하도록 할 수 있음  
  * 
...
* 잉여 속성 체크는 오류를 찾는 효과적인 방법이지만,   
  **TS 타입 체커가 수행하는 일반적인 구조적 할당 가능성 체크와 역할이 다름**    
  할당의 개념을 정확히 알아야 둘을 구분 가능함    
  * 
...
* 만약 같은 타입 시그니처를 반복적으로 작성한 코드가 있을 경우  
  함수 타입을 분리하거나 이미 존재하는 타입을 찾아보도록함  
  라이브러리를 직접 만들 경우 공통 콜백에 타입을 제공해야 함  
* 다른 함수의 시그니처를 참조하려면 typeof fn을 사용하면 됨  


## 13. 타입과 인터페이스의 차이점 알기
* TS에서 명명된 타입을 정의하는 방법은 2가지가 존재함  
  ```ts
  ...
  //인터페이스 대신 클래스 사용도 가능하긴함  
  ```

<br> 

* 대부분의 경우 타입이나 인터페이스를 둘 다 사용해도 되지만, 차이점을 정확히 알고   
  같은 상황에서는 동일한 방법으로 명명된 타입을 정의해 일관성을 유지해야 함  

<br> 

* 인터페이스와 타입 선언의 비슷한 점  
  * 명명된 타입은 인터페이스로 정의했든 타입으로 정의했든 상태에는 차이가 없음  
...
* 프로젝트에서 어떤 문법을 사용할지 결정할 때 한 가지 일관된 스타일을 확립하고,  
  보강 기법이 필요한지 고려해야 함  
